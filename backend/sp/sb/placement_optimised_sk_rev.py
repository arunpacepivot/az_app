...
def process_campaign_placement(placement_df: pd.DataFrame, target_acos: float) -> pd.DataFrame:
    ...
    metrics_to_aggregate: List[str] = [
        "Impressions", "Clicks", "Spend", "14 Day Total Sales (₹)", 
    ...
    placement_df["ACOS"]=placement_df["Spend"]/placement_df["14 Day Total Sales (₹)"]
    ...
    combined_df.loc[:, "ACOS"] = combined_df["Spend"] / combined_df["14 Day Total Sales (₹)"].replace(0, float("nan"))
    ...
    overall_rpc = combined_df["14 Day Total Sales (₹)"].sum() / combined_df["Clicks"].sum() if combined_df["Clicks"].sum() > 0 else 0.0

    # Calculate Top of Search RPC
    top_of_search_sales_sum = placement_df[placement_df["Placement"] == "Top of Search on Amazon"]["14 Day Total Sales (₹)"].sum()
    ...
    rest_of_search_sales_sum = placement_df[placement_df["Placement"] == "Rest of Search on Amazon"]["14 Day Total Sales (₹)"].sum()
    ...
    condition = (combined_df["14 Day Total Sales (₹)"] > 0) & (combined_df["Clicks"] > 0)

    combined_df.loc[condition, "RPC"] = (
        combined_df.loc[condition, "14 Day Total Sales (₹)"] / 
    ...
    for asin in combined_df["ASIN"].unique():
        ...
        asin_rpc_tos = combined_df.loc[asin_condition_tos, "14 Day Total Sales (₹)"].sum() / combined_df.loc[asin_condition_tos, "Clicks"].sum() if combined_df.loc[asin_condition_tos, "Clicks"].sum() > 0 else 0.0
        ...
        asin_rpc_ros = combined_df.loc[asin_condition_ros, "14 Day Total Sales (₹)"].sum() / combined_df.loc[asin_condition_ros, "Clicks"].sum() if combined_df.loc[asin_condition_ros, "Clicks"].sum() > 0 else 0.0
