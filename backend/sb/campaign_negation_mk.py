...
def campaign_negation_mk(bulk_df, str_df, target_acos):
    bulk_df = bulk_df.copy()
    str_df = str_df.copy()

     # Grouped summary of ASIN and Placement
    str_summary = str_df.groupby(["Campaign Name (Informational only)"]).agg({
        "Impressions": "sum",
        "Clicks": "sum",
        "Spend": "sum",
        "Sales": "sum",
    ...
    if not filtered_df.empty:
        ...
        # Set tgt column value based on Match Type or Product Targeting Expression
        filtered_df["tgt"] = filtered_df.apply(
            lambda row: row["Match Type"] if pd.notna(row["Match Type"]) and row["Match Type"] != "" else row["Product Targeting Expression"],
            axis=1
        )
        
        filtered_df = filtered_df[
        (filtered_df["tgt"] != "EXACT") & 
        (~filtered_df["tgt"].str.startswith("asin")) 
        ]
    ...
    if "Campaign Name" not in pt_df.columns:
        print("Campaign Name not found in pt_df")
    ...
    if "Campaign Name (Informational only)" not in kw_df.columns:
        print("Campaign Name not found in kw_df")
    else:
